...
def computeScore(queryObj,entityObj,structure,lucene_handler,conf_paras):
    ...
    if IS_SAS_USED==True and entityObj.categories is not None:
       ...
       elif MODEL_NAME in ['fsdm','sdm']:
          text_sim=fsdm_sas(queryObj,entityObj,structure,lucene_handler)
          if text_sim==0.0 or text_sim==NEGATIVE_INFINITY:
             if MODEL_NAME=='sdm':
    ...
    if IS_WIKI_DOC_TREE_USED==True:
          has_wiki=True
          # option 1: complete model proposed in the paper 
          if entityObj.wiki_doc_tree is not None: 
             wiki_sim=scoreWikiTree(queryObj,entityObj.wiki_doc_tree,lucene_handler['wiki'],entityObj.wiki_doc_tree.used_content_field)
             if wiki_sim==NEGATIVE_INFINITY:
                has_wiki=False
          '''
          # option 2: a special case of the model proposed in the paper
          # best practice that trade-off between performance and time
          if entityObj.wiki_doc_tree is not None:  
              list_str_node=[]
              entityObj.wiki_doc_tree.getSubTreePathContentByDepth(2,list_str_node)
              list_section=[List_Term_Object(item,False,' ',mongoObj,w2vmodel) for item in list_str_node]
              #list_section=[List_Term_Object(item,False,' ',mongoObj,w2vmodel) for item in entityObj.wiki_doc_tree.getSubTreeContentByDepth(2)]
              has_wiki=True
              try:
                 wiki_sim=max([lmSim(queryObj.contents_obj,section_obj,entityObj.wiki_doc_tree.used_content_field,w2vmodel,'log_sum',lucene_handler['wiki']) for section_obj in list_section])
              except Exception as e:
                 has_wiki=False
          '''       
    # end computing text_sim
    if has_wiki==True:
       score=0.85*score+0.15*wiki_sim
    return score
